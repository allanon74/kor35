...
Le **immagini rubriche** (e altri media) non viaggiano nel sync DB: solo path nel JSON + file via rsync. Timer media orario (`*:15`). Dopo upload sul master, se il Pi ha articoli senza foto: `make sync-media` subito.
