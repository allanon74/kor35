...
class GestioneOggettiService:
    ...
    def crea_oggetto_da_infusione(infusione, proprietario, nome_personalizzato=None):
        ...
        CORRETTO: Non passa 'proprietario' al costruttore, ma usa sposta_in_inventario.
        """
        ...
        # Crea l'oggetto (senza assegnare ancora il proprietario nel costruttore)
        ...
        # Copia Caratteristiche (Componenti)
    ...
    def verifica_competenza_assemblaggio(personaggio, host, componente):
        """Verifica requisiti per Installazione/Rimozione."""
        punteggi = personaggio.caratteristiche_base
        
        # Verifica Mattoni (Componenti)
    ...
    def assembla_mod(assemblatore, oggetto_host, potenziamento, check_skills=True):
        """Gestisce l'installazione di un mod."""
        # Controlli di base
    ...
    def rimuovi_mod(assemblatore, host, mod, check_skills=True):
        ...
        if mod not in host.potenziamenti_installati.all():
            raise ValidationError("Modulo non installato.")
        ...
        with transaction.atomic():
            ...
            mod.sposta_in_inventario(proprietario_items) # Torna al proprietario dell'host
            mod.save()
            host.save()
    ...
    def elabora_richiesta_assemblaggio(richiesta_id, esecutore):
        """
        Esegue la richiesta di lavoro (Accettazione).
        """
        from .models import RichiestaAssemblaggio, STATO_RICHIESTA_PENDENTE, STATO_RICHIESTA_COMPLETATA, TIPO_OPERAZIONE_FORGIATURA, TIPO_OPERAZIONE_RIMOZIONE
        
        try: 
            req = RichiestaAssemblaggio.objects.select_related('committente', 'artigiano', 'oggetto_host', 'componente', 'infusione').get(pk=richiesta_id)
        ...
        if req.stato != STATO_RICHIESTA_PENDENTE: 
             raise ValidationError("Già processata.")
        ...
        with transaction.atomic():
            ...
            if req.tipo_operazione == TIPO_OPERAZIONE_FORGIATURA:
                # AVVIA TIMER: L'artigiano lavora, il destinatario è il committente
