...
from .models import (
    # Costanti
    CARATTERISTICA, STATISTICA, AURA, ELEMENTO,
    STATO_TRANSAZIONE_IN_ATTESA, STATO_TRANSAZIONE_ACCETTATA, STATO_TRANSAZIONE_RIFIUTATA,
    STATO_TRANSAZIONE_CHOICES,
    
    # Modelli Core
    Punteggio, Caratteristica, Statistica, Aura, Mattone, ModelloAura,
    Tabella, Tier,
    
    # Modelli Items
    Abilita, Attivata, Infusione, Tessitura, Oggetto,
    
    # Modelli Through/Statistiche
    CaratteristicaModificatore,
    AbilitaStatistica, MattoneStatistica,
    OggettoStatistica, OggettoStatisticaBase, OggettoElemento,
    InfusioneStatisticaBase, InfusioneMattone,
    TessituraStatisticaBase, TessituraMattone,
    AttivataStatisticaBase, AttivataElemento,
    
    # Personaggio e Link
    Personaggio, TipologiaPersonaggio,
    PersonaggioAbilita, PersonaggioAttivata, PersonaggioInfusione, PersonaggioTessitura, PersonaggioModelloAura,
    CreditoMovimento, PuntiCaratteristicaMovimento, PersonaggioLog,
    TransazioneSospesa,
...
# Factory per evidenziare i valori modificati
def create_stat_form(model_class, field_valore, field_default_stat):
    class StatForm(forms.ModelForm):
        ...
        def __init__(self, *args, **kwargs):
            ...
            # Evidenzia se il valore è diverso dal default della statistica
...
class IconaAdminMixin:
    def icona_html(self, obj):
        return obj.icona_html
    icona_html.short_description = "Icona"
    icona_html.allow_tags = True
    
    def icona_cerchio_html(self, obj):
        return obj.icona_cerchio_html
    icona_cerchio_html.short_description = "Cerchio"
    icona_cerchio_html.allow_tags = True

    def icona_cerchio_inverted_html(self, obj):
        return obj.icona_cerchio_inverted_html
    icona_cerchio_inverted_html.short_description = "Cerchio Inv."
    icona_cerchio_inverted_html.allow_tags = True


# --- LOGICA PIVOT (AUTO-POPOLAMENTO) ---

class StatisticaPivotMixin:
    """
    Mixin che popola automaticamente le righe delle statistiche mancanti nel formset.
    """
    ...
    def has_delete_permission(self, request, obj=None):
        return False # Impedisce di cancellare le statistiche (devono esserci tutte)
    ...
    def get_formset(self, request, obj=None, **kwargs):
        if obj is not None: 
            ...
            # Trova le statistiche già associate
            ...
            for stat in missing_stats:
                # Prende il default corretto (Base o Normale)
                default_val = getattr(stat, self.default_source_field, 0)
                
                kwargs_init = {
                    fk_name: obj,
                    'statistica': stat,
                    self.value_field: default_val
                }
...
# 1. PIVOT BASE (Tabular, Semplice) -> Per "Statistiche Base"
...
# 2. PIVOT MODIFICATORE (Stacked, Condizionale) -> Per "Statistiche Modificatori"
class StatisticaModificatorePivotInline(StatisticaPivotMixin, admin.StackedInline):
    ...
    # Widget per selezione multipla comoda
    ...
    fieldsets = (
        ...
        ('Condizioni di Applicazione', {
            ...
            'description': "Se i flag sono attivi, il modificatore si applica SOLO se la condizione è soddisfatta."
        }),
    )
...
def get_statistica_base_help_text():
    return mark_safe("<b>Variabili Valori Base disponibili:</b><br>Inserire {parametro} nel testo.")

def get_mattone_help_text():
    return mark_safe(get_statistica_base_help_text() + "<br><b>Variabili Speciali Mattone:</b> {caratt}, {3*caratt}.")
...
class AbilitaAdmin(A_Admin):
    ...
    # ABILITA HA SOLO MODIFICATORI (Pivot Condizionale)
    inlines = (
        abilita_tier_inline, 
...
class MattoneAdmin(A_Admin):
    ...
    list_filter = ('aura', 'caratteristica_associata')
    search_fields = ('nome',)
    summernote_fields = ('descrizione_mattone', 'descrizione_metatalento', 'testo_addizionale')
    
    # MATTONE HA SOLO MODIFICATORI (Pivot Condizionale)
    inlines = [MattoneStatisticaInline]
    
    # FIX: Rimosso 'tipo' dai fieldsets
    fieldsets = (
        ('Dati Mattone', {
            'fields': ('nome', 'aura', 'caratteristica_associata', 'descrizione_mattone', 'icona', 'colore')
        }),
        ('Metatalento', {
            'fields': ('funzionamento_metatalento', 'descrizione_metatalento', 'testo_addizionale')
        }),
        ('Sistema', {
            'fields': ('sigla',), 'classes': ('collapse',)
        })
    )
...
class OggettoAdmin(SModelAdmin):
    ...
    # OGGETTO HA ENTRAMBE: BASE (Semplice) e MODIFICATORI (Condizionali)
...
class InfusioneAdmin(SModelAdmin):
    ...
    # INFUSIONE HA SOLO BASE (Pivot Semplice)
...
class TessituraAdmin(SModelAdmin):
    ...
    # TESSITURA HA SOLO BASE (Pivot Semplice)
...
class PersonaggioAdmin(A_Admin):
    ...
    fieldsets = (
        ('Info', {
            'fields': ('nome', 'proprietario', 'tipologia', 'testo', ('data_nascita', 'data_morte'))
        }),
        ('Valori', {
            'classes': ('collapse',), 
            'fields': (('id', 'data_creazione'), ('crediti', 'punti_caratteristica'))
        })
    )
