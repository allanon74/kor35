...
class PunteggioAdminForm(forms.ModelForm):
    class Meta: model = Punteggio; fields = '__all__'; widgets = {'icona': CustomIconWidget}

# Forms
class AbilitaStatisticaForm(forms.ModelForm):
    class Meta: model = AbilitaStatistica; fields = '__all__'
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk and self.instance.statistica and self.instance.valore != self.instance.statistica.valore_predefinito:
             for f in self.fields: self.fields[f].widget.attrs['style'] = HIGHLIGHT_STYLE
class MattoneStatisticaForm(forms.ModelForm):
    class Meta: model = MattoneStatistica; fields = '__all__'
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance.pk and self.instance.statistica and self.instance.valore != self.instance.statistica.valore_predefinito:
             for f in self.fields: self.fields[f].widget.attrs['style'] = HIGHLIGHT_STYLE
class OggettoStatisticaBaseForm(forms.ModelForm):
    class Meta: model = OggettoStatisticaBase; fields = '__all__'
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class OggettoStatisticaForm(forms.ModelForm):
    class Meta: model = OggettoStatistica; fields = '__all__'
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance.pk and self.instance.statistica and self.instance.valore != self.instance.statistica.valore_predefinito:
             for f in self.fields: self.fields[f].widget.attrs['style'] = HIGHLIGHT_STYLE
class AttivataStatisticaBaseForm(forms.ModelForm):
    class Meta: model = AttivataStatisticaBase; fields = '__all__'
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        if self.instance.pk and self.instance.statistica and self.instance.valore_base != self.instance.statistica.valore_base_predefinito:
             for f in self.fields: self.fields[f].widget.attrs['style'] = HIGHLIGHT_STYLE
...
class A_Admin(SModelAdmin):
    actions_on_top = True; save_on_top = True
...
class StatisticaPivotInlineBase(admin.TabularInline):
    readonly_fields = ('statistica',); extra = 0
...
class AbilitaStatisticaInline(StatisticaPivotInlineBase):
    model = AbilitaStatistica; form = AbilitaStatisticaForm; fk_name = 'abilita'; value_field = 'valore'; default_field = 'valore_predefinito'
class MattoneStatisticaInline(StatisticaPivotInlineBase):
    model = MattoneStatistica; form = MattoneStatisticaForm; fk_name = 'mattone'; value_field = 'valore'; default_field = 'valore_predefinito'
...
class PersonaggioModelloAuraInline(admin.TabularInline):
    model = PersonaggioModelloAura; extra = 1
...
def get_statistica_base_help_text():
    ...
    except Exception as e: return format_html(f"<b style='color:red;'>Errore: {e}</b>")
...
class TipologiaPersonaggioAdmin(admin.ModelAdmin): list_display = ('nome', 'crediti_iniziali', 'caratteristiche_iniziali', 'giocante')
...
class PunteggioAdmin(A_Admin):
    ...
    list_filter = ('tipo', 'caratteristica_relativa'); list_editable = ('ordine','tipo'); search_fields = ('nome',); summernote_fields = ('descrizione',)
    save_as = True
...
class ModelloAuraAdmin(admin.ModelAdmin): list_display = ('nome', 'aura'); list_filter = ('aura',); filter_horizontal = ('mattoni_proibiti',)
...
class MattoneAdmin(A_Admin):
    ...
    search_fields = ('nome', 'testo_addizionale'); summernote_fields = ('descrizione_mattone', 'descrizione_metatalento', 'testo_addizionale')
    inlines = [MattoneStatisticaInline]
    fieldsets = (('Dati Mattone', {'fields': ('nome', 'aura', 'caratteristica_associata', 'tipo', 'descrizione_mattone', 'icona', 'colore')}),
...
class AbilitaPrerequisitoAdmin(A_Admin): list_display = ('abilita', 'prerequisito'); search_fields = ['abilita__nome', 'prerequisito__nome']; autocomplete_fields = ['abilita', 'prerequisito']
@admin.register(Tier)
class TierAdmin(A_Admin): list_display = ['nome', 'descrizione']; summernote_fields = ["descrizione"]; inlines = [abilita_tier_inline]; save_as = True
@admin.register(Statistica)
class StatisticaAdmin(A_Admin): form = PunteggioAdminForm; list_display = ('nome', 'parametro', 'is_primaria', 'valore_predefinito', 'valore_base_predefinito', 'tipo_modificatore'); list_editable = ('is_primaria',); exclude = ('tipo',); summernote_fields = ('descrizione',)
...
class StatisticaCondizionaleInlineBase(admin.StackedInline):
    ...
    fieldsets = (
        ...
        ('Condizioni', {'classes': ('collapse',), 'fields': (('usa_limitazione_aura', 'limit_a_aure'), ('usa_limitazione_elemento', 'limit_a_elementi'))}),
    )
    def has_delete_permission(self, request, obj=None): return False
    def get_max_num(self, request, obj=None, **kwargs): return Statistica.objects.count()
    # Pivot Logic (Duplicate of above but needed for StackedInline)
    def get_formset(self, request, obj=None, **kwargs):
        if obj is not None: 
            fk_name = self.fk_name
            all_stats = Statistica.objects.all()
...
class ManifestoAdmin(SModelAdmin): list_display = ('id', 'data_creazione', 'nome'); readonly_fields = ('id', 'data_creazione'); summernote_fields = ['testo']
@admin.register(QrCode)
class QrCodeAdmin(admin.ModelAdmin): list_display = ('id', 'data_creazione'); readonly_fields = ('id', 'data_creazione'); summernote_fields = ['testo']
...
class OggettoAdmin(SModelAdmin):
    ...
    fieldsets = (('Info', {'fields': ('nome', 'aura', 'testo', ('id', 'data_creazione', 'livello'))}), ('Anteprima', {'classes': ('wide',), 'fields': ('mostra_testo_formattato',)}))
    inlines = [PunteggioOggettoInline, OggettoStatisticaBaseInline, OggettoStatisticaInline] # Correct order: Base, then Modifiers
...
class AttivataStatisticaBaseInline(StatisticaPivotInlineBase):
    model = AttivataStatisticaBase; form = AttivataStatisticaBaseForm; fk_name = 'attivata'; value_field = 'valore_base'; default_field = 'valore_base_predefinito'
...
class AttivataAdmin(SModelAdmin):
    ...
    inlines = [AttivataStatisticaBaseInline, PunteggioAttivataInline]; exclude = ('statistiche_base', 'elementi'); summernote_fields = ['testo']
...
class InfusioneAdmin(SModelAdmin):
    ...
    inlines = [InfusioneMattoneInline, InfusioneStatisticaBaseInline, InfusioneStatisticaInline] # Base + Modifiers
...
class TessituraAdmin(SModelAdmin):
    ...
    readonly_fields = ('livello', 'mostra_testo_formattato', 'id', 'data_creazione')
    inlines = [TessituraMattoneInline, TessituraStatisticaBaseInline, TessituraStatisticaInline] # Base + Modifiers
    ...
    def get_form(self, request, obj=None, **kwargs):
        form = super().get_form(request, obj, **kwargs)
        if 'testo' in form.base_fields: form.base_fields['testo'].help_text = get_statistica_base_help_text()
        if 'formula' in form.base_fields: form.base_fields['formula'].help_text += " Supporta {elem}, {rango}."
        return form
    def mostra_testo_formattato(self, obj): return format_html("{}", mark_safe(obj.TestoFormattato))
