...
Il timer esegue `scripts/mirror_media_sync.sh`: **push best-effort** poi **pull obbligatorio**. Se il push verso master fallisce (es. `Permission denied` su directory legacy di proprietà root), il pull da master continua comunque — non usare più `make sync-media-push && make sync-media` nelle unit.
