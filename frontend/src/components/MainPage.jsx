...
/** Loader dinamici: usati sia da React.lazy sia dal preload su hover/touch. */
const TAB_LOADERS = {
  home: () => import('./HomeTab.jsx'),
  watch: () => import('./WatchTab.jsx'),
  admin_msg: () => import('./AdminMessageTab.jsx'),
  inventario: () => import('./InventoryTab.jsx'),
  abilita: () => import('./AbilitaTab.jsx'),
  tessiture: () => import('./TessitureTab.jsx'),
  infusioni: () => import('./InfusioniTab.jsx'),
  cerimoniali: () => import('./CerimonialiTab.jsx'),
  consumabili: () => import('./ConsumabiliTab.jsx'),
  negozi: () => import('./NegoziMercanteTab.jsx'),
  ...
  personaggi: () => import('./PersonaggiTab.jsx'),
...
const AVAILABLE_TABS = [
    { id: 'inventario', label: 'Inventario', icon: Backpack, component: lazyTab('inventario') },
    { id: 'abilita', label: 'Abilità', icon: Zap, component: lazyTab('abilita') },
    { id: 'tessiture', label: 'Tessiture', icon: Scroll, component: lazyTab('tessiture') },
    { id: 'infusioni', label: 'Infusioni', icon: TestTube2, component: lazyTab('infusioni') },
    { id: 'cerimoniali', label: 'Cerimoniali', icon: Users, component: lazyTab('cerimoniali') },
    { id: 'consumabili', label: 'Consumabili', icon: Package, component: lazyTab('consumabili') },
    { id: 'negozi', label: 'Negozi', icon: Store, component: lazyTab('negozi'), requiresModulo: 'negozi' },
    ...
    { id: 'personaggi', label: 'Personaggi', icon: Users, component: lazyTab('personaggi') },
...
const MainPage = ({ token, onLogout, onSwitchToMaster }) => {
  ...
  const sharedNowTs = useSharedNowTs();
  const [toasts, setToasts] = useState([]);
  
  // STATO SHORTCUTS (Default salvagente)
  const [userShortcuts, setUserShortcuts] = useState(DEFAULT_SHORTCUTS);

  // Preload chunk della tab attiva + pin di default (Edge Wi‑Fi).
  useEffect(() => {
    preloadTabChunk(activeTab);
    DEFAULT_SHORTCUTS.forEach(preloadTabChunk);
    preloadTabChunk('home');
  }, [activeTab]);
  ...
  const revivalRemaining = Math.max(0, Math.ceil((revivalEndMs - sharedNowTs) / 1000));
  ...
  const renderTabContent = useCallback(() => {
    let content = null;
    if (activeTab === 'home') {
      content = <HomeTab onLogout={onLogout} />;
    } else if (activeTab === 'game') {
      content = <GameTab onNavigate={handleMenuNavigation} />;
      ...
      if (tabDef) {
          ...
          if (tabDef.id === 'personaggi') {
          ...
          } else if (tabDef.id === 'qr') {
              content = <Component onScanSuccess={handleScanSuccess} onLogout={onLogout} isStealingOnCooldown={isStealingOnCooldown} cooldownTimer={cooldownTimer} />;
          } else if (tabDef.id === 'logs' || tabDef.id === 'transazioni') {
              content = <Component charId={selectedCharacterId} onLogout={onLogout} />;
          } else if (tabDef.id === 'messaggi') {
              content = (
                  <Component
                      onLogout={onLogout}
              ...
              content = <Component onLogout={onLogout} />;
          }
      } else {
          content = <HomeTab onLogout={onLogout} />;
      }
  ...
  return (
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative">
          ...
          <header className="relative flex justify-between items-center p-3 shadow-md shrink-0 border-b border-gray-700 z-10 h-16 backdrop-blur-sm bg-gray-800/95">
              ...
              <div className="flex items-center gap-3 z-20">
                  ...
                  <button
                    ...
                    className="md:hidden relative p-2 rounded-full hover:bg-gray-700 transition-colors text-gray-200"
          ...
          {isMenuOpen && (
            <div className="md:hidden fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-label="Menu navigazione">
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm transition-opacity" onClick={() => setIsMenuOpen(false)} aria-hidden="true"></div>
                
                <div id="main-mobile-drawer" className="relative w-85 max-w-sm bg-gray-800 h-full shadow-2xl flex flex-col border-l border-gray-700 animate-slide-in-right overflow-hidden">
                    <div className="flex justify-between items-center p-4 border-b border-gray-700 bg-gray-900/50 shrink-0 h-16">
                        <h2 className="text-lg font-bold text-gray-100 uppercase tracking-wider flex items-center gap-2">
                            <UserCog size={20} className="text-indigo-400" aria-hidden="true"/> Opzioni
                        </h2>
                        <button type="button" onClick={() => setIsMenuOpen(false)} className="text-gray-400 hover:text-white p-2" aria-label="Chiudi menu"><X size={24} aria-hidden="true" /></button>
                    </div>
                    {/* Riutilizza il contenuto del menu */}
                    {renderMenuContent()}
                </div>
          ...
          {/* --- BOTTOM NAV --- */}
          <nav
            className="flex justify-around items-center bg-gray-800 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.3)] shrink-0 border-t border-gray-700 z-10 h-[60px] pb-safe"
          ...
          >
            ...
            {selectedCharacterId && watchTabEnabled ? (
              <TabButton
                ...
                onPrefetch={() => preloadTabChunk('watch')}
              />
            ) : null}
            ...
            {userShortcuts.map(tabId => {
                ...
                if (tab.id === 'messaggi') {
                    if (hasStaffMsgNotif) {
                        ...
                        dotLabel = 'Messaggi staff non letti';
                    } else if (hasMsgNotif) {
                        showDot = true;
                        dotColor = 'bg-purple-500';
                        dotLabel = 'Messaggi non letti';
                    }
                }
                ...
                return (
                    <TabButton 
                        ...
                        onPrefetch={() => preloadTabChunk(tabId)}
                        notificationDot={showDot}
                        dotColor={dotColor}
                        notificationLabel={dotLabel}
                    />
                );
            })}
          </nav>
...
const TabButton = memo(({ icon, label, isActive, onClick, onPrefetch, notificationDot, dotColor, notificationLabel }) => (
  <button 
    type="button"
    onClick={onClick}
  ...
  >
    <div className={`transition-transform duration-200 ${isActive ? '-translate-y-1 scale-110' : 'group-active:scale-95'}`}>
        ...
        {notificationDot && (
            <span
              className={`absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border border-gray-800 animate-pulse ${dotColor}`}
              aria-label={notificationLabel || 'Notifica'}
              role="status"
            />
        )}
    </div>
    ...
    {isActive && (
        <div className="absolute bottom-0 w-8 h-0.5 bg-indigo-400 rounded-t-full shadow-[0_0_8px_rgba(99,102,241,0.6)] animate-pulse" aria-hidden="true"></div>
    )}
  </button>
));
