...
import { changePassword, setArcanaLocalPassword } from '../api';
...
export default function PasswordChangeModal({ isOpen, onClose, onLogout, forceSetMode = false, onSuccess = null }) {
  ...
  const isDirty = Boolean(oldPassword || newPassword || confirmPassword);
  const requestClose = useDirtyModalClose(
    isDirty && !loading && message.type !== 'success',
    onClose,
    'Hai inserito dati nella password. Chiudere comunque?'
  );
