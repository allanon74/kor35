...
export default function MissioniManager({ onLogout }) {
  ...
  const korpOptions = useMemo(
    () => [
      { value: null, label: '— Nessuna (task generica) —' },
      ...korps.map((k) => ({
        value: k.id,
  ...
  const eventoOptions = useMemo(
    () => (eventi || []).map((ev) => ({ value: ev.id, label: ev.titolo })),
    [eventi],
  );
  ...
  return (
    <div className="h-full overflow-y-auto bg-gray-950 p-4 text-gray-100">
          ...
          {items.map((row) => (
            ...
            >
              ...
              <div className="mt-1 flex flex-wrap gap-2 text-[11px] text-gray-400">
                ...
                {row.korp_nome ? (
                  <span className="rounded bg-violet-900/50 px-2 py-0.5 text-violet-200">{row.korp_nome}</span>
                ) : (
                ...
                {!row.attiva ? <span className="text-amber-400">Inattiva</span> : null}
              </div>
            </StaffListRow>
          ))}
          {items.length === 0 ? (
            <li className="text-sm text-gray-500 list-none">Nessuna task. Creane una.</li>
          ) : null}
        </ul>
      )}
      ...
      <StaffModal
        open={modalOpen}
        title={form.id ? 'Modifica task' : 'Nuova task'}
      ...
      >
        <StaffSection title="Anagrafica">
          <StaffFieldGrid>
            ...
            <LabeledField label="KORP" hint="Vuoto = task non di Korp">
              <SearchableSelect
                options={korpOptions}
                value={form.korp}
            ...
            <LabeledField label="Tipo risoluzione">
              <select
                className={staffInputClass()}
                value={form.tipo_risoluzione}
                onChange={(e) => setForm({ ...form, tipo_risoluzione: e.target.value })}
              >
                {TIPO_OPTS.map((o) => (
        ...
        <StaffSection title="Premi base">
          <StaffFieldGrid>
            <LabeledField label="Crediti (Cr)">
              <input
                ...
                step="0.01"
                className={staffInputClass()}
                value={form.reward_crediti}
                onChange={(e) => setForm({ ...form, reward_crediti: e.target.value })}
              />
            </LabeledField>
            <LabeledField label="Prestigio (Pr)">
              <input
                ...
                min={0}
                className={staffInputClass()}
                value={form.reward_prestigio}
                onChange={(e) => setForm({ ...form, reward_prestigio: e.target.value })}
              />
            </LabeledField>
          </StaffFieldGrid>
          <label className="mt-2 flex items-center gap-2 text-sm text-gray-300">
            <input
              type="checkbox"
              checked={!!form.premio_solo_primo}
              onChange={(e) => setForm({ ...form, premio_solo_primo: e.target.checked })}
            />
            Premio solo al primo risolvitore
          </label>
            ...
            <StaffFieldGrid>
              <LabeledField label="Malus Cr se non primo">
                <input
                  ...
                  step="0.01"
                  className={staffInputClass()}
                  value={form.malus_non_primo_crediti}
                  onChange={(e) => setForm({ ...form, malus_non_primo_crediti: e.target.value })}
                />
              </LabeledField>
              <LabeledField label="Malus Pr se non primo">
                <input
                  type="number"
                  className={staffInputClass()}
                  value={form.malus_non_primo_prestigio}
                  onChange={(e) => setForm({ ...form, malus_non_primo_prestigio: e.target.value })}
                />
              </LabeledField>
              <LabeledField label="Bonus Cr risoluzioni successive">
                <input
                  type="number"
                  step="0.01"
                  className={staffInputClass()}
                  value={form.bonus_successive_crediti}
                  onChange={(e) => setForm({ ...form, bonus_successive_crediti: e.target.value })}
                />
              </LabeledField>
              <LabeledField label="Bonus Pr risoluzioni successive">
                <input
                  type="number"
                  className={staffInputClass()}
                  value={form.bonus_successive_prestigio}
                  onChange={(e) => setForm({ ...form, bonus_successive_prestigio: e.target.value })}
                />
              </LabeledField>
            </StaffFieldGrid>
          ) : null}
        ...
        <StaffSection title="Eventi associati">
          <div className="max-h-48 space-y-1 overflow-y-auto rounded border border-gray-700 p-2">
            {eventoOptions.length === 0 ? (
              ...
              eventoOptions.map((ev) => (
                <label key={ev.value} className="flex items-center gap-2 text-sm text-gray-200">
                  <input
                    type="checkbox"
                    checked={(form.eventi_ids || []).map(Number).includes(Number(ev.value))}
