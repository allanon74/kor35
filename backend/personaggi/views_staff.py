...
class ManifestoStaffViewSet(viewsets.ModelViewSet):
    """CRUD manifesti (contenuto in `testo`, requisiti JSON opzionali, audio/video).
...
class PersonaggioStaffViewSet(viewsets.ModelViewSet):
    ...
    def assegna_abilita(self, request, pk=None):
        ...
        result = acquisisci_abilita_personaggio(
            personaggio,
            abilita_id,
            request,
            staff=True,
    ...
    def rimuovi_abilita(self, request, pk=None):
        """Revoca un'abilità con validazioni giocatore e rimborso PC/crediti."""
        ...
        result = revoca_abilita_personaggio(
            personaggio,
            abilita_id,
            staff=True,
            motivo_staff=motivo,
        ...
        if not result.ok:
            ...
            if result.error and (
                'automaticamente' in result.error
                or 'prerequisito' in result.error
                or 'non revocabile' in result.error.lower()
            ):
