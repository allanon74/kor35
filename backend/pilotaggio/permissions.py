...
class IsCompattatoreConsole(permissions.BasePermission):
    """Token console valido + statistica ingegneria (0IN) sul personaggio."""

    message = "Accesso console ingegneria non autorizzato."

    def has_permission(self, request, view):
        from .navigation_stats import ingegneria_stat_sigla
        from .models import PilotRuntimeConfig
        ...
        if pilota is None:
            return False
        if not cfg.compattatore_login_richiesto:
            return True
        return int(pilota.get_valore_statistica(ingegneria_stat_sigla(cfg)) or 0) > 0
